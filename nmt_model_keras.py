from keras.layers import Embedding,LSTM,Dropout,Dense,Layer,Permute,Dot,Softmax,Multiply
...
class AttentionLayer(Layer):
  ...
  def call(self, inputs, mask=None):
    ...
    Start
    """
    # transpose the last two dimensions of decoder_outputs
    # permute_dimensions = Permute((2, 1))
    decoder_outputs_T = K.permute_dimensions(decoder_outputs, (0, 2, 1))
    print("output shape: ", decoder_outputs.shape)
    # compute dot product of decoder_outputs and encoder_outputs
    # batch_dot = Dot(axes=[2, 1])
    luong_score = K.batch_dot(decoder_outputs_T, encoder_outputs, axes=[1, 2])
    print("luong shape: ", luong_score.shape)
    soft_max = Softmax(axis=1)
    luong_score = soft_max(luong_score)
    ...
    print("encoder vector shape: ", encoder_vector.shape)
    ...
    print("final shape: ", new_decoder_outputs.shape)
...
class NmtModel(object):
  ...
  def build(self):
    ...
    embeddings_source = Embedding(self.vocab_source_size, self.embedding_size, embeddings_initializer='random_uniform', mask_zero=True,) 
    embeddings_target = Embedding(self.vocab_target_size, self.embedding_size, embeddings_initializer='random_uniform', mask_zero=True,) 
    ...
    # dropout_source = Dropout(rate=self.embedding_dropout_rate)
    source_words_embeddings = embeddings_source(source_words)
    source_words_embeddings = Dropout(rate=self.embedding_dropout_rate)(source_words_embeddings)
    target_words_embeddings = embeddings_target(target_words)
    target_words_embeddings = Dropout(rate=self.embedding_dropout_rate)(target_words_embeddings)

    # (c.) An encoder LSTM() with return sequences set to True
    print('\nTask 1(c): Creating an encoder')
    encoder_lstm = LSTM(units=self.hidden_size, return_state=True, return_sequences=True, recurrent_dropout=self.hidden_dropout_rate)
